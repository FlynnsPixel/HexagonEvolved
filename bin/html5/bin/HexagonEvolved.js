...
ApplicationMain.preloader_onComplete = function(event) {
	...
	if(Reflect.field(core.build.game.Main,"main") == null) {
	...
	} else Reflect.field(core.build.game.Main,"main").apply(core.build.game.Main,[]);
}
var flash = {}
flash.events = {}
...
core.build = {}
core.build.game = {}
core.build.game.Main = function() {
	flash.display.Sprite.call(this);
};
$hxClasses["core.build.game.Main"] = core.build.game.Main;
core.build.game.Main.__name__ = ["core","build","game","Main"];
core.build.game.Main.main = function() {
	...
	flash.Lib.get_current().addChild(new core.build.game.Main());
	core.build.game.managers.Universe.initiate();
}
core.build.game.Main.__super__ = flash.display.Sprite;
core.build.game.Main.prototype = $extend(flash.display.Sprite.prototype,{
	__class__: core.build.game.Main
});
var DocumentClass = function() {
	core.build.game.Main.call(this);
};
$hxClasses["DocumentClass"] = DocumentClass;
DocumentClass.__name__ = ["DocumentClass"];
DocumentClass.__super__ = core.build.game.Main;
DocumentClass.prototype = $extend(core.build.game.Main.prototype,{
	get_stage: function() {
...
var DefaultAssetLibrary = function() {
	...
	DefaultAssetLibrary.type.set("assets/openfl.svg",value);
	DefaultAssetLibrary.path.set("assets/sound/chipzel.mp3","assets/sound/chipzel.mp3");
	var value = Reflect.field(openfl.AssetType,"music".toUpperCase());
	DefaultAssetLibrary.type.set("assets/sound/chipzel.mp3",value);
	DefaultAssetLibrary.path.set("assets/sound/chipzel.ogg","assets/sound/chipzel.ogg");
	var value = Reflect.field(openfl.AssetType,"music".toUpperCase());
	DefaultAssetLibrary.type.set("assets/sound/chipzel.ogg",value);
	DefaultAssetLibrary.path.set("assets/sound/juicyfruit.mp3","assets/sound/juicyfruit.mp3");
	var value = Reflect.field(openfl.AssetType,"music".toUpperCase());
	DefaultAssetLibrary.type.set("assets/sound/juicyfruit.mp3",value);
	DefaultAssetLibrary.path.set("assets/sound/juicyfruit.ogg","assets/sound/juicyfruit.ogg");
	var value = Reflect.field(openfl.AssetType,"music".toUpperCase());
	DefaultAssetLibrary.type.set("assets/sound/juicyfruit.ogg",value);
	DefaultAssetLibrary.path.set("assets/sound/chipzel.mp3","assets/sound/chipzel.mp3");
	var value = Reflect.field(openfl.AssetType,"music".toUpperCase());
	DefaultAssetLibrary.type.set("assets/sound/chipzel.mp3",value);
	DefaultAssetLibrary.path.set("assets/sound/chipzel.ogg","assets/sound/chipzel.ogg");
...
core.build.game.entities = {}
core.build.game.entities.Ship = function() {
	this.movedelaytimer = 0;
	this.moving = false;
	this.dest = 0;
	flash.display.Sprite.call(this);
};
$hxClasses["core.build.game.entities.Ship"] = core.build.game.entities.Ship;
core.build.game.entities.Ship.__name__ = ["core","build","game","entities","Ship"];
core.build.game.entities.Ship.__super__ = flash.display.Sprite;
core.build.game.entities.Ship.prototype = $extend(flash.display.Sprite.prototype,{
	endmovement: function() {
		this.moving = false;
		this.movedelaytimer = 0;
		this.set_rotation(this.dest);
	}
	,update: function() {
		if(this.moving) {
			this.set_rotation(this.dest);
			++this.movedelaytimer;
			if(this.movedelaytimer >= 6) {
				this.movedelaytimer = 0;
				this.moving = false;
			}
		} else if(core.build.game.managers.Universe.keyboard.rightkeydown) {
			this.dest += core.build.game.managers.Universe.stagemanager.increment * (180 / Math.PI);
			this.moving = true;
		} else if(core.build.game.managers.Universe.keyboard.leftkeydown) {
			this.dest -= core.build.game.managers.Universe.stagemanager.increment * (180 / Math.PI);
			this.moving = true;
		}
		this.base.set_y(core.build.game.managers.Universe.stagemanager.hexagon.get_height() / 1.5);
	}
	,create: function() {
		this.base = new flash.display.Shape();
		this.base.get_graphics().lineStyle(1,0);
		this.base.get_graphics().beginFill(6645093);
		this.base.get_graphics().lineTo(15,0);
		this.base.get_graphics().lineTo(7.5,15);
		this.base.get_graphics().lineTo(0,0);
		this.base.set_x(-(this.base.get_width() / 2));
		this.base.set_y(core.build.game.managers.Universe.stagemanager.hexagon.get_height());
		this.base.set_filters([new flash.filters.DropShadowFilter(4,90,0,1,4,4,1)]);
		this.container = new flash.display.Sprite();
		this.container.addChild(this.base);
		this.addChild(this.container);
		core.build.game.managers.Universe.stagemanager.container.addChild(this);
	}
	,__class__: core.build.game.entities.Ship
});
core.build.game.managers = {}
core.build.game.managers.AssetManager = function() {
	this.soundtransform = new flash.media.SoundTransform();
	this.juicyfruit = this.getsound("assets/sound/juicyfruit");
	this.chipzel = this.getsound("assets/sound/chipzel");
};
$hxClasses["core.build.game.managers.AssetManager"] = core.build.game.managers.AssetManager;
core.build.game.managers.AssetManager.__name__ = ["core","build","game","managers","AssetManager"];
core.build.game.managers.AssetManager.prototype = {
	playsound: function(id) {
		this.soundchannel = core.build.game.managers.Universe.assets.juicyfruit.play(0,99,this.soundtransform);
	}
	,getsound: function(name) {
		return openfl.Assets.getSound(name + ".ogg");
	}
	,__class__: core.build.game.managers.AssetManager
}
core.build.game.managers.KeyboardManager = function() {
	this.rightkeydown = false;
	this.leftkeydown = false;
	this.downkeydown = false;
	this.upkeydown = false;
};
$hxClasses["core.build.game.managers.KeyboardManager"] = core.build.game.managers.KeyboardManager;
core.build.game.managers.KeyboardManager.__name__ = ["core","build","game","managers","KeyboardManager"];
core.build.game.managers.KeyboardManager.prototype = {
	keyup: function(event) {
		var key = event.keyCode;
		if(key == 65 || key == 37) {
			core.build.game.managers.Universe.player.endmovement();
			this.leftkeydown = false;
		}
		if(key == 68 || key == 39) {
			core.build.game.managers.Universe.player.endmovement();
		...
		if(key == 87 || key == 38) this.upkeydown = false;
		if(key == 83 || key == 40) this.downkeydown = false;
	}
	,keydown: function(event) {
		var key = event.keyCode;
		if(key == 65 || key == 37) this.leftkeydown = true;
		if(key == 68 || key == 39) this.rightkeydown = true;
		if(key == 87 || key == 38) this.upkeydown = true;
		if(key == 83 || key == 40) this.downkeydown = true;
	}
	,initiate: function() {
		core.build.game.managers.Universe.stage.addEventListener(flash.events.KeyboardEvent.KEY_DOWN,$bind(this,this.keydown));
		core.build.game.managers.Universe.stage.addEventListener(flash.events.KeyboardEvent.KEY_UP,$bind(this,this.keyup));
	}
	,__class__: core.build.game.managers.KeyboardManager
}
core.build.game.managers.MouseManager = function() {
	this.mouseisdown = false;
};
$hxClasses["core.build.game.managers.MouseManager"] = core.build.game.managers.MouseManager;
core.build.game.managers.MouseManager.__name__ = ["core","build","game","managers","MouseManager"];
core.build.game.managers.MouseManager.prototype = {
	...
	,initiate: function() {
		core.build.game.managers.Universe.stage.addEventListener(flash.events.MouseEvent.MOUSE_DOWN,$bind(this,this.mousedown));
		core.build.game.managers.Universe.stage.addEventListener(flash.events.MouseEvent.MOUSE_UP,$bind(this,this.mouseup));
	}
	,__class__: core.build.game.managers.MouseManager
}
core.build.game.managers.ToolManager = function() {
	this.animations = flash._Vector.Vector_Impl_._new();
	this.trails = flash._Vector.Vector_Impl_._new();
};
$hxClasses["core.build.game.managers.ToolManager"] = core.build.game.managers.ToolManager;
core.build.game.managers.ToolManager.__name__ = ["core","build","game","managers","ToolManager"];
core.build.game.managers.ToolManager.prototype = {
	flip: function(data) {
		var flippedata = new flash.display.BitmapData(data.___textureBuffer != null?data.___textureBuffer.width:0,data.___textureBuffer != null?data.___textureBuffer.height:0,true,0);
		var matrix = new flash.geom.Matrix(-1,0,0,1,data.___textureBuffer != null?data.___textureBuffer.width:0,0);
		flippedata.draw(data,matrix);
		data.dispose();
		data = null;
		return flippedata;
	}
	,remove: function(object,from) {
	...
	,createtrail: function(image,x,y,fadespeed,autoupdate) {
		if(autoupdate == null) autoupdate = true;
		if(fadespeed == null) fadespeed = .1;
		var trail = new core.build.game.tools.Trail(image,x,y,fadespeed);
		if(autoupdate) flash._Vector.Vector_Impl_.push(this.trails,trail);
		return trail;
	}
	,cutspritesheet: function(sheet,width,height) {
		var frames = flash._Vector.Vector_Impl_._new();
		var rowx = (sheet.___textureBuffer != null?sheet.___textureBuffer.width:0) / width;
		var rowy = (sheet.___textureBuffer != null?sheet.___textureBuffer.height:0) / height;
		...
		while(_g1 < _g) {
			var y = _g1++;
			var _g3 = 0, _g2 = rowx | 0;
			while(_g3 < _g2) {
				var x = _g3++;
				var data = new flash.display.BitmapData(width | 0,height | 0,true,0);
				rect.x = Math.round(x * width);
				rect.y = Math.round(y * height);
				data.copyPixels(sheet,rect,point);
				flash._Vector.Vector_Impl_.push(frames,data);
			}
		}
		return frames;
	}
	,createanimation: function(frames,rate,applyto,autoupdate) {
		if(autoupdate == null) autoupdate = true;
		var animation = new core.build.game.tools.Animation(frames,rate,applyto);
		if(autoupdate) flash._Vector.Vector_Impl_.push(this.animations,animation);
		return animation;
	}
	,update: function() {
		var _g1 = 0, _g = flash._Vector.Vector_Impl_.get_length(this.animations);
		while(_g1 < _g) {
			var i = _g1++;
			if(i <= flash._Vector.Vector_Impl_.get_length(this.animations) - 1) this.animations[i].update();
		}
		var _g1 = 0, _g = flash._Vector.Vector_Impl_.get_length(this.trails);
		while(_g1 < _g) {
			var i = _g1++;
			if(i <= flash._Vector.Vector_Impl_.get_length(this.trails) - 1) this.trails[i].update();
		}
	}
	,__class__: core.build.game.managers.ToolManager
}
core.build.game.managers.Universe = function() { }
$hxClasses["core.build.game.managers.Universe"] = core.build.game.managers.Universe;
core.build.game.managers.Universe.__name__ = ["core","build","game","managers","Universe"];
core.build.game.managers.Universe.initiate = function() {
	core.build.game.managers.Universe.stage = flash.Lib.get_current().get_stage();
	core.build.game.managers.Universe.stage.addEventListener(flash.events.Event.ENTER_FRAME,core.build.game.managers.Universe.update);
	core.build.game.managers.Universe.keyboard = new core.build.game.managers.KeyboardManager();
	core.build.game.managers.Universe.keyboard.initiate();
	core.build.game.managers.Universe.mouse = new core.build.game.managers.MouseManager();
	core.build.game.managers.Universe.mouse.initiate();
	core.build.game.managers.Universe.tools = new core.build.game.managers.ToolManager();
	core.build.game.managers.Universe.assets = new core.build.game.managers.AssetManager();
	core.build.game.managers.Universe.player = new core.build.game.entities.Ship();
	core.build.game.managers.Universe.stagemanager = new core.build.game.stage.StageManager();
	core.build.game.managers.Universe.stagemanager.create();
	core.build.game.managers.Universe.player.create();
}
core.build.game.managers.Universe.update = function(event) {
	core.build.game.managers.Universe.player.update();
	core.build.game.managers.Universe.stagemanager.update();
}
...
core.build.game.stage = {}
core.build.game.stage.Obstacle = function() {
	this.speed = 5;
	this.removed = false;
	...
	this.midy = 0;
	this.midx = 0;
	flash.display.Shape.call(this);
};
$hxClasses["core.build.game.stage.Obstacle"] = core.build.game.stage.Obstacle;
core.build.game.stage.Obstacle.__name__ = ["core","build","game","stage","Obstacle"];
core.build.game.stage.Obstacle.__super__ = flash.display.Shape;
core.build.game.stage.Obstacle.prototype = $extend(flash.display.Shape.prototype,{
	update: function() {
		if(core.build.game.managers.Universe.stagemanager.musicmode) {
			this.distancex -= core.build.game.managers.Universe.stagemanager.average * this.speed;
			this.distancey -= core.build.game.managers.Universe.stagemanager.average * this.speed;
		} else {
			this.distancex -= this.speed;
			this.distancey -= this.speed;
		}
		if(this.distancex <= -this.speed - 10 || this.distancey <= -this.speed - 10) {
			this.distancex = 0;
			this.distancey = 0;
			this.removed = true;
		}
		var angle = core.build.game.managers.Universe.stagemanager.increment + this.offsetrotation;
		this.get_graphics().clear();
		this.get_graphics().lineStyle(1,this.colour);
		this.get_graphics().beginFill(this.colour);
		this.get_graphics().moveTo(this.midx + Math.cos(angle) * this.distancex,this.midy + Math.sin(angle) * this.distancey);
		angle += core.build.game.managers.Universe.stagemanager.increment;
		this.get_graphics().lineTo(this.midx + Math.cos(angle) * this.distancex,this.midy + Math.sin(angle) * this.distancey);
		this.get_graphics().lineTo(this.midx + Math.cos(angle) * (this.distancex + this.length),this.midy + Math.sin(angle) * (this.distancey + this.length));
		angle -= core.build.game.managers.Universe.stagemanager.increment;
		this.get_graphics().lineTo(this.midx + Math.cos(angle) * (this.distancex + this.length),this.midy + Math.sin(angle) * (this.distancey + this.length));
		this.get_graphics().lineTo(this.midx + Math.cos(angle) * this.distancex,this.midy + Math.sin(angle) * this.distancey);
		this.get_graphics().endFill();
		var playerrotation = core.build.game.managers.Universe.player.get_rotation() + core.build.game.managers.Universe.stagemanager.incrementangle | 0;
		if(playerrotation >= 180) playerrotation = -playerrotation;
		console.log(playerrotation);
		if(180 - (angle - core.build.game.managers.Universe.stagemanager.increment) * (180 / Math.PI) == playerrotation) {
		}
	}
	,__class__: core.build.game.stage.Obstacle
});
core.build.game.stage.StageManager = function() {
	...
	this.sectors = 6;
	this.container = new flash.display.Sprite();
	var size = core.build.game.managers.Universe.stage.get_stageWidth() * 1.25 | 0;
	if(core.build.game.managers.Universe.stage.get_stageHeight() >= size) size = core.build.game.managers.Universe.stage.get_stageHeight() * 1.25 | 0;
	this.rect = new flash.geom.Rectangle(0,0,size,size);
	this.point = new flash.geom.Point(0,0);
	this.obstacles = flash._Vector.Vector_Impl_._new();
};
$hxClasses["core.build.game.stage.StageManager"] = core.build.game.stage.StageManager;
core.build.game.stage.StageManager.__name__ = ["core","build","game","stage","StageManager"];
core.build.game.stage.StageManager.prototype = {
	createobstacle: function(obstaclerotation) {
		var obstacle = new core.build.game.stage.Obstacle();
		obstacle.midx = this.midx;
		obstacle.midy = this.midy;
		flash._Vector.Vector_Impl_.push(this.obstacles,obstacle);
		this.container.addChild(obstacle);
		this.container.setChildIndex(this.hexagon,this.container.__children.length - 1);
		obstacle.set_x(-(this.rect.width / 2));
		obstacle.set_y(-(this.rect.height / 2));
		obstacle.distancex = this.midx * 1.5;
		obstacle.distancey = this.midy * 1.5;
		obstacle.offsetrotation += this.increment * obstaclerotation;
		var colour = 9803157;
		if(obstaclerotation % 2 == 0) colour = 7829367;
		obstacle.colour = colour;
	}
	,update: function() {
		this.colourchanger.update();
		var _g = this.container;
		_g.set_rotation(_g.get_rotation() + this.average / 2);
		++this.obstacletimer;
		if(this.obstacletimer >= 40) {
			this.obstacletimer = 0;
			var obstaclerotation = Math.random() * this.sectors | 0;
			var _g1 = 0, _g = (Math.random() * (this.sectors - 1) | 0) + 1;
			while(_g1 < _g) {
				var i = _g1++;
		...
		var index = 0;
		var _g1 = 0, _g = flash._Vector.Vector_Impl_.get_length(this.obstacles);
		while(_g1 < _g) {
			var i = _g1++;
			this.obstacles[index].update();
			if(this.obstacles[index].removed) {
				this.container.removeChild(this.obstacles[index]);
				this.obstacles[index].get_graphics().clear();
				flash._Vector.Vector_Impl_.splice(this.obstacles,index,1);
				--index;
			}
			++index;
		}
	...
	,draw: function() {
		this.midx = this.rect.width / 2 | 0;
		this.midy = this.rect.height / 2 | 0;
		var angle = this.increment;
		var graphics = new flash.display.Shape();
		this.hexagon = new flash.display.Shape();
		this.hexagon.get_graphics().lineStyle(1.5,0);
		this.hexagon.get_graphics().beginFill(6645093);
		this.hexagon.set_filters([new flash.filters.DropShadowFilter(4,90,0,1,4,4,1)]);
		var _g1 = 0, _g = this.sectors;
		while(_g1 < _g) {
			var i = _g1++;
			var colour = 6645093;
			if(i % 2 == 0) colour = 2697513;
			graphics.get_graphics().lineStyle(1,colour);
			graphics.get_graphics().beginFill(colour);
			graphics.get_graphics().moveTo(this.midx,this.midy);
			graphics.get_graphics().lineTo(this.midx + Math.cos(angle) * (this.midx * 2.8),this.midy + Math.sin(angle) * (this.midy * 2.8));
			...
			if(i == 0) {
				angle -= this.increment;
				this.hexagon.get_graphics().moveTo(Math.cos(angle) * 20,Math.sin(angle) * 20);
				angle += this.increment;
			}
			this.hexagon.get_graphics().lineTo(Math.cos(angle) * 20,Math.sin(angle) * 20);
		}
		this.hexagon.get_graphics().endFill();
		core.build.game.managers.Universe.stage.addChild(this.hexagon);
		this.container.addChild(this.hexagon);
		this.canvasdata.fillRect(this.rect,0);
		this.canvasdata.draw(graphics);
		graphics.get_graphics().clear();
		this.canvas.set_x(-(this.rect.width / 2 | 0));
		this.canvas.set_y(-(this.rect.height / 2 | 0));
		this.container.set_x(core.build.game.managers.Universe.stage.get_stageWidth() / 2);
		this.container.set_y(core.build.game.managers.Universe.stage.get_stageHeight() / 2);
	}
	,create: function() {
		this.canvasdata = new flash.display.BitmapData(this.rect.width | 0,this.rect.height | 0,true,0);
		this.canvas = new flash.display.Bitmap(this.canvasdata);
		core.build.game.managers.Universe.stage.addChild(this.container);
		this.container.addChild(this.canvas);
		this.increment = 360 / this.sectors / 2 / (180 / Math.PI) * 2;
		this.incrementangle = Math.round(this.increment * (180 / Math.PI));
		this.colourchanger = new core.build.game.tools.ColourChanger(this.container,.005,2.5);
		this.draw();
		core.build.game.managers.Universe.assets.playsound(0);
		flash.external.ExternalInterface.call("tab.open","http://www.google.com","_blank");
	}
	,__class__: core.build.game.stage.StageManager
}
core.build.game.tools = {}
core.build.game.tools.Animation = function(animationframes,animationrate,applytobase) {
	this.loop = true;
	this.pause = false;
	this.rate = 0;
	this.timer = 0;
	...
	this.index = Math.random() * (flash._Vector.Vector_Impl_.get_length(this.frames) - 1) | 0;
	this.updateframe();
};
$hxClasses["core.build.game.tools.Animation"] = core.build.game.tools.Animation;
core.build.game.tools.Animation.__name__ = ["core","build","game","tools","Animation"];
core.build.game.tools.Animation.prototype = {
	updateframe: function() {
		if(this.index >= flash._Vector.Vector_Impl_.get_length(this.frames) && this.loop) this.index = 0;
		if(this.index >= 0 && this.index <= flash._Vector.Vector_Impl_.get_length(this.frames) - 1) this.target.set_bitmapData(this.frames[this.index]);
	...
	,__class__: core.build.game.tools.Animation
}
core.build.game.tools.ColourChanger = function(applyTo,colourSpeed,maxItensity) {
	if(maxItensity == null) maxItensity = 1.4;
	if(colourSpeed == null) colourSpeed = .002;
	this.reversed = false;
	this.maxrgb = 0;
	this.rgbspeed = 0;
	this.blueAdd = 0;
	...
	this.transform = new flash.geom.ColorTransform(this.maxrgb,1,1);
	this.redAdd = this.rgbspeed;
};
$hxClasses["core.build.game.tools.ColourChanger"] = core.build.game.tools.ColourChanger;
core.build.game.tools.ColourChanger.__name__ = ["core","build","game","tools","ColourChanger"];
core.build.game.tools.ColourChanger.prototype = {
	revert: function() {
		if(this.redAdd > 0) {
			this.greenAdd = 0;
			this.redAdd = -this.rgbspeed;
			this.blueAdd = this.rgbspeed;
		} else if(this.blueAdd > 0) {
			this.redAdd = 0;
			this.blueAdd = -this.rgbspeed;
			this.greenAdd = this.rgbspeed;
		} else if(this.greenAdd > 0) {
			this.redAdd = this.rgbspeed;
			this.blueAdd = 0;
			this.greenAdd = -this.rgbspeed;
		}
	...
	,update: function() {
		this.transform.redMultiplier += this.redAdd;
		this.transform.greenMultiplier += this.greenAdd;
		this.transform.blueMultiplier += this.blueAdd;
		if(this.transform.redMultiplier >= this.maxrgb) {
			this.transform.redMultiplier = this.maxrgb;
			this.transform.greenMultiplier = 1;
			if(this.reversed) this.reverse(); else this.revert();
		} else if(this.transform.blueMultiplier >= this.maxrgb) {
			this.transform.redMultiplier = 1;
			this.transform.blueMultiplier = this.maxrgb;
			if(this.reversed) this.reverse(); else this.revert();
		} else if(this.transform.greenMultiplier >= this.maxrgb) {
			this.transform.greenMultiplier = this.maxrgb;
			this.transform.blueMultiplier = 1;
			if(this.reversed) this.reverse(); else this.revert();
		}
		if(this.container != null) this.container.transform.set_colorTransform(this.transform);
	}
	,reset: function() {
		this.container.transform.set_colorTransform(this.transform);
	}
	,__class__: core.build.game.tools.ColourChanger
}
core.build.game.tools.Trail = function(image,posx,posy,speed) {
	this.rotation = 0;
	this.beginalpha = 1;
	this.movemap = false;
	this.pause = false;
...
$hxClasses["core.build.game.tools.Trail"] = core.build.game.tools.Trail;
core.build.game.tools.Trail.__name__ = ["core","build","game","tools","Trail"];
core.build.game.tools.Trail.prototype = {
	removeall: function() {
		var _g1 = 0, _g = flash._Vector.Vector_Impl_.get_length(this.trails);
		while(_g1 < _g) {
			var i = _g1++;
			this.trails[i].parent.removeChild(this.trails[i]);
		}
		this.trails = [];
	}
	,update: function() {
		if(!this.pause) {
			var trail = new flash.display.Sprite();
			var base = new flash.display.Bitmap(this.trailimage);
			trail.addChild(base);
			trail.set_x(this.x + base.get_width() / 2);
			trail.set_y(this.y + base.get_height() / 2);
			base.set_x(-(base.get_width() / 2));
			base.set_y(-(base.get_height() / 2));
			trail.alpha = this.beginalpha;
			core.build.game.managers.Universe.stage.addChild(trail);
			flash._Vector.Vector_Impl_.push(this.trails,trail);
			var _g1 = 0, _g = flash._Vector.Vector_Impl_.get_length(this.trails);
			while(_g1 < _g) {
				var i = _g1++;
				if(i <= flash._Vector.Vector_Impl_.get_length(this.trails) - 1) {
					this.trails[i].alpha -= this.fadespeed;
					var _g2 = this.trails[i];
					_g2.set_rotation(_g2.get_rotation() + (this.trails[i].get_rotation() - this.rotation));
					if(this.trails[i].alpha <= 0) {
						this.trails[i].parent.removeChild(this.trails[i]);
						flash._Vector.Vector_Impl_.splice(this.trails,i,1);
					}
				}
			}
		}
	}
	,__class__: core.build.game.tools.Trail
}
var haxe = {}
haxe.Timer = function() { }
...
$hxClasses["flash.external.ExternalInterface"] = flash.external.ExternalInterface;
flash.external.ExternalInterface.__name__ = ["flash","external","ExternalInterface"];
flash.external.ExternalInterface.addCallback = function(functionName,closure) {
	flash.Lib.addCallback(functionName,closure);
}
flash.external.ExternalInterface.call = function(functionName,p1,p2,p3,p4,p5) {
...
flash.filters.BlurFilter.prototype = $extend(flash.filters.BitmapFilter.prototype,{
	...
	,__preFilter: function(surface) {
		var ctx = surface.getContext("2d");
		this.__kernel = flash._Vector.Vector_Impl_._new();
...
flash.external.ExternalInterface.available = true;
...
//@ sourceMappingURL=HexagonEvolved.js.map